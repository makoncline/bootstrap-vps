...
9. Confirm `~/.codex/skills/server-new-app/SKILL.md` and `~/.codex/skills/server-state-audit/SKILL.md` exist for the server operator user.
...
15. Confirm `swapon --show` lists `/swapfile` and `sysctl vm.swappiness` reports `10`.
16. Confirm `~/.codex/config.toml` exists if Codex has not already created one, and that it enables web search plus workspace-write network access.
17. For later app deploys, remember that changes under `/srv/stacks/proxy/sites` take effect by restarting the proxy container, not by calling `caddy reload`, because the managed Caddy config sets `admin off`.
18. If deploy webhook credentials were provided, confirm `systemctl status bootstrap-deploy-webhook` is healthy and `GET https://<DEPLOY_WEBHOOK_HOSTNAME>/healthz` returns success through the tunnel.
19. If a config-sync target is configured for an app, confirm the server can fetch the app repo at a specific commit SHA into `/var/lib/bootstrap-config-sync/<target>` and apply `deploy/vps/` without overwriting the live stack `.env`.
